...
var database = configuration["POSTGRES_DATABASE"] ??
               throw new InvalidOperationException("POSTGRES_DATABASE not configured");
...
// var esCloudId = configuration["ELASTIC_CLOUD_ID"] ??
//                 throw new InvalidOperationException("ELASTIC_CLOUD_ID not configured");
// var esApiKey = configuration["ELASTIC_API_KEY"] ??
//                throw new InvalidOperationException("ELASTIC_API_KEY not configured");
//
//
...
var cloudId = configuration["ELASTIC_CLOUD_ID"] ??
              throw new InvalidOperationException("ELASTIC_CLOUD_ID not configured");
var apiKeyBase64 = configuration["ELASTIC_API_KEY"] ??
                   throw new InvalidOperationException("ELASTIC_API_KEY not configured");

var decodedApiKey = System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(apiKeyBase64));

// Create ApiKeyAuthenticationCredentials
var apiKeyCredentials = new ApiKeyAuthenticationCredentials(decodedApiKey);

// Create CloudConnectionPool with credentials and date time provider
var connectionPool = new CloudConnectionPool(cloudId, apiKeyCredentials, DateTimeProvider.Default);

// Create connection settings with the pool
var connectionSettings = new ConnectionSettings(connectionPool)
    .DefaultIndex("dls-exam1");

// Create ElasticClient
var elasticClient = new ElasticClient(connectionSettings);

builder.Services.AddSingleton<ElasticClient>(elasticClient);
...
void ConfigureRabbitMqInfrastructure(IApplicationBuilder webApp)
{
    ...
    catch (Exception ex)
    {
        logger.LogCritical(ex,
            "Order Service: CRITICAL - An unhandled error occurred while configuring RabbitMQ infrastructure during startup.");
    }
}
